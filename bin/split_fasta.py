...
#    print(str(keys))
#    for b in keys:
#        print(str(len(keys))+" key: "+str(b)+" counter: "+str(counter))
#        if dic[b]:
...
#        print(dic.keys())
    #reformat keys
    return {"%04d"%key:value for (key,value) in dic.items()}

def write(dic):
    for b in dic.keys():
#        print("writing to file "+str(b)+".fasta")
        fh = open(str(b)+".fasta", "w")
        fh.write(dic[b])
        fh.close()


if __name__ == '__main__':
    #set random see to some number - I choose '42' for no particular reason ;-)
    random.seed(42)
